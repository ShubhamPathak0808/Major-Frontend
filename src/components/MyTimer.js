...
const MyTimer = (props) => {                                     /* new added part starts */
    ...
    useEffect(() => {                                                //new part
        ...
        const fetchDuration = async () => {
          try {
            const res = await Axios.get(`http://10.25.101.103:8000/api/attendence/${attendenceId}`);
            if (res.data.success) {
...
const MyTimer = ({ endAttendence }) => {
  ...
  useEffect(() => {
    ...
    const fetchDuration = async () => {
      try {
        const res = await Axios.get(`http://10.25.101.103:8000/api/attendance/${attendenceId}`);
        if (res.data.success) {
          setTimeLeft(res.data.data.duration);
        }
      } catch (error) {
