...
export const generateExcelTotal = (attendences, courseStudents) => {
	...
	for (const item of attendences) {
		promises.push(Axios.get(`http://10.25.100.77:8000/api/attendenceResult/${item._id}`));
	}
...
const Attendence = ({ history }) => {
	...
	const qr_url = `http://10.25.100.77:3000/attendence/${qr}`; //new part added
	...
	React.useEffect(() => {
		...
		Axios.get(`http://10.25.100.77:8000/api/attendence/${attendenceId}`).then(
			(res) => {
				if (res.data.success) {
	...
	React.useEffect(() => {
		...
		if (userType === "student") {
			Axios.get(
				`http://10.25.100.77:8000/api/attendence/hasSubmitted/${attendenceId}/${user._id}`
			).then((res) => {
				if (res.data.data) {
	...
	React.useEffect(() => {
		...
		let attendenceId = loc[loc.length - 1];
		Axios.get(
			`http://10.25.100.77:8000/api/attendenceResult/${attendenceId}`
		).then((res) => {
			if (res.data.success) {
	...
	//     try {
	//       const res = await Axios.get(`http://10.25.100.77:8000/api/attendenceResult/${attendenceId}`);
	//       if (res.data.success) {
	//         setAttendenceResults(res.data.data);
	//       }
	//     } catch (error) {
	...
	const submitAttendence = () => {
		...
		Axios.post("http://10.25.100.77:8000/api/submitAttendence", attendenceResponse)
			.then((res) => {
				if (res.data.success) {
				} else {
	...
	const startAttendence = () => {
		...
		forceUpdate();
		Axios.post(
			`http://10.25.100.77:8000/api/startAttendence/${attendenceId}`
		).then((res) => {
			if (res.data.success) {
			}
		});
	...
	const endAttendence = () => {
		...
		let attendenceId = loc[loc.length - 1];
		forceUpdate();
		Axios.post(`http://10.25.100.77:8000/api/endAttendence/${attendenceId}`).then(
			(res) => {
				if (res.data.success) {
				}
			}
