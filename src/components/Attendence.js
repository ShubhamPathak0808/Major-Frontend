...
import { Copy, Download, Grid, PlayCircle, XCircle } from "react-feather";
  ...
  Promise.allSettled(promises).then((values) => {
    ...
    for (const AttendanceResults of values) {
      ...
      courseStudents.forEach((ticket) => {
        let name = ticket.fName + " " + ticket.lName;
        if (
          AttendanceResults.value.data.data.some(
            (person) => person.student_name === name
          )
        )
          newColumnValues.push("Present");
        else newColumnValues.push("Absent");
      });
    ...
    >
      ...
      <div //check for generating pdf button and updated part for generating excel sheet
        ...
        style={{
          ...
          display:
            userType === "teacher" || userType === "hod" ? "flex" : "none",
        }}
        onClick={() =>
      ...
      {userType === "teacher" ? (
        <React.Fragment>
          ...
          {/* string part will be added here */}
          <div
            style={{
              position: "absolute",
              top: 100,
