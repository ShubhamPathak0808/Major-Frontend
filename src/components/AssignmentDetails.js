...
const AssignmentDetails = () => {
  ...
  React.useEffect(() => {
    Axios.get(`http://10.25.101.103:8000/api/assignment/${assignmentID}`)
      .then((res) => {
        let a = res.data.data;
        const course_id = a.course_id;

        Axios.get(`http://10.25.101.103:8000/api/${course_id}`)
          .then((res) => {
  ...
  React.useEffect(() => {
    ...
    Axios.get(`http://10.25.101.103:8000/api/marks/${assignmentID}`)
      .then((res) => {
        let a = res.data;
        console.log("courseId", courseId);
        Axios.get(`http://10.25.101.103:8000/api/assignment/students/${assignmentID}`)
          .then((res) => {
  ...
  const sendMarks = () => {
    ...
    Axios.post(
      `http://10.25.101.103:8000/api/submission/grade/${assignmentID}/${currentStudent.student_id}`,
      {
        marks: marks,
      }
    )
  ...
  const submitAssignment = () => {
    ...
    Axios.post(`http://10.25.101.103:8000/api/submission`, {
      assignment_id: assignmentID,
      student_id: user._id,
    })
      .then((res) => {
        Axios.post(
          `http://10.25.101.103:8000/api/submission/attachment/${res.data.data._id}`,
          formData,
          {
            headers: {
  ...
  const checkAttachment = () => {
    Axios.get(`http://10.25.101.103:8000/api/hasAttachment/${assignmentID}`)
      .then((res) => {
        if (res.data.success) {
  ...
  const checkHasSubmitted = () => {
    Axios.get(`http://10.25.101.103:8000/api/hasSubmitted/${assignmentID}/${user._id}`)
      .then((res) => {
        ...
        >
          ...
          {hasAttachment === true ? (
            <reactFragment>
              <br />
              <a
                href={`http://10.25.101.103:8000/api/assignment/attachment/${assignmentID}`}
                target="_blank"
              >
                <button
                  className="btn btn-new"
                  ...
                  {studentSubmissions.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <a
                            href={`http://10.25.101.103:8000/api/submission/getAttachment/${assignmentID}/${item.student_id}`}
                            target="_blank"
                          >
                            <Download
                              size={20}
