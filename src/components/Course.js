...
const Course = () => {
  ...
  const { qr, setqr } = useContext(qrContext);      //new added part
	...
	React.useEffect(() => {
		...
		let courseID = arr[arr.length - 1];
		let courseInfo = null;
		Axios.get(`http://10.25.101.103:8000/api/course/${courseID}`)
			.then((res) => {
				if (res.data.success) {
	...
	React.useEffect(() => {
		Axios.get(`http://10.25.101.103:8000/api/quiz/course/${courseID}`)
			.then((res) => {
				if (res.data.success) {
	...
	React.useEffect(() => {
		Axios.get(`http://10.25.101.103:8000/api/attendence/course/${courseID}`)
			.then((res) => {
	...
	React.useEffect(() => {
		...
		Axios.get(`http://10.25.101.103:8000/api/teacher/${teacher_id}`)
			.then((res) => {
				if (res.data.success) {
	...
	React.useEffect(() => {
		Axios.get(`http://10.25.101.103:8000/api/course/assignment/${courseID}`)
			.then((res) => {
				if (res.data.success) {
	...
	React.useEffect(() => {
		...
		Axios.get(`http://10.25.101.103:8000/api/messages/${loc[loc.length - 1]}`).then(
			(res) => {
				if (res.data.success) {
	...
	const postMaterial = () => {
		if (isAssignment) {
			...
			Axios.post("http://10.25.101.103:8000/api/attendence", attendenceData).then(
				(res) => {
					console.log(res);
		...
		Axios.post("http://10.25.101.103:8000/api/assignment", materialData)
			.then((res) => {
				var formData = new FormData();
				formData.append("file", attachment);
				Axios.post(
					`http://10.25.101.103:8000/api/assignment/attachment/${res.data.data._id}`,
					formData,
					{
						headers: {
					...
					.catch((err) => {
						if (isAssignment === false) {
							...
							Axios.delete(
								`http://10.25.101.103:8000/api/assignment/${res.data.data._id}`
							)
								.then(() => {
	...
	const removeStudent = (student_id, course_id) => {
		Axios.post("http://10.25.101.103:8000/api/removeStudent", {
	...
	const deleteCourse = (course_code, course_id) => {
		Axios.delete(`http://10.25.101.103:8000/api/course/${course_id}`)
			.then((res) => {
				if (res.data.success) {
	...
	const changeCourseName = () => {
		...
		const url = `http://10.25.101.103:8000/api/course/changeName/${courseID}`;
		Axios.post(url, {
			name: newCourseName,
		})
			.then((res) => {
	...
	const sendMessage = () => {
		...
		Axios.post(`http://10.25.101.103:8000/api/message`, obj).then((res) => {
			if (res.data.success) {
			}
			Axios.get(
				`https://10.25.101.103:8000/api/messages/${loc[loc.length - 1]}`
			).then((res) => {
				if (res.data.success) {
					setMessages(res.data.data);
				}
