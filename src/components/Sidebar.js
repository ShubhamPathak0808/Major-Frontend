...
const Sidebar = () => {
  ...
  React.useEffect(() => {
    Axios.get(`http://10.25.100.17:8000/api/${userType}/${user._id}`, {
      header: {
  ...
  React.useEffect(() => {
    ...
    Axios.get(`http://10.25.100.17:8000/api/fetchCourse/student/${user._id}`, {
      header: {
        "Content-Type": "application/json; charset=utf-8",
      },
    })
  ...
  React.useEffect(() => {
    ...
    Axios.get(`http://10.25.100.17:8000/api/fetchCourse/teacher/${user._id}`, {
      header: {
        "Content-Type": "application/json; charset=utf-8",
      },
    })
  ...
  const changeName = () => {
    ...
    const url = `http://10.25.100.17:8000/api/update/${userType}`;
    Axios.post(url, {
      fName: newFName,
      lName: newLName,
      email: user.email,
